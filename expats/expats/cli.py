...
def main():
    ...
    evaluate_parser.add_argument("artifact_path", type=str, help="path to training artifacts")
    ...
    predict_parser.add_argument("artifact_path", type=str, help="path to training artifacts")
    ...
    interpret_parser.add_argument("artifact_path", type=str, help="path to training artifacts")
    ...
    if args.mode == "train":
        ...
        if args.overrides:
            config = merge_with_dotlist(config, args.overrides)
        logger.info(f"Loaded config: {config}")
        evaluate(config, args.artifact_path)
        ...
        if args.overrides:
            config = merge_with_dotlist(config, args.overrides)
        logger.info(f"Loaded config: {config}")
        predict(config, args.artifact_path, args.output_path)
        ...
        if args.overrides:
            config = merge_with_dotlist(config, args.overrides)
        logger.info(f"Loaded config: {config}")
        interpret(config, args.artifact_path)
