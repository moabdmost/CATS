...
def load_texts(data_file, expected_size=None):
    ...
    for line in tqdm(open(data_file), total=expected_size, desc=f'Loading {data_file}'):
        texts.append(json.loads(line)['essay'])

    return texts


class Corpus:
    def __init__(self, name, data_dir='/workspace/data', skip_train=False):
        # download(name, data_dir=data_dir)
        self.name = name
        self.train = load_texts(f'{data_dir}/{name}_train.jsonl') if not skip_train else None
        self.test = load_texts(f'{data_dir}/{name}_test.jsonl')
        self.valid = load_texts(f'{data_dir}/{name}_valid.jsonl')
